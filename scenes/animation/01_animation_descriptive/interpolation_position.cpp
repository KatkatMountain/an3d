...
void scene_model::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    */
    ...
    // ...
    // Preparation of data for the cardinal spline interpolation
    // Parameters used to compute the cardinal spline interpolation
    const int idx3 = std::min(idx+2, int(keyframes.size()-1));
    
    const float t0 = keyframes[0].t; // = t_{i-1}
    const float t1 = keyframes[idx  ].t; // = t_i
    const float t2 = keyframes[idx+1].t; // = t_{i+1}
    const float t3 = keyframes[idx3].t; // = t_{i+2}

    const vec3& p0 = keyframes[0].p; // = p_{i-1}
    const vec3& p1 = keyframes[idx  ].p; // = p_i
    const vec3& p2 = keyframes[idx+1].p; // = p_{i+1}
    const vec3& p3 = keyframes[idx3].p; // = p_{i+2}
...
{
    const float mu = 1;
    const vec3 di = mu * (p2 - p0) / (t2 - t0);
    const vec3 di_1 = mu * (p3 - p1) / (t3 - t1);
    const float s = (t - t1)/(t2 - t1);
    
    const vec3 p = (2*s*s*s - 3*s*s + 1) * p1 + (s*s*s - 2*s*s + s) * di
        + (-2*s*s*s + 3*s*s) * p2 + (s*s*s - s*s) * di_1;;
